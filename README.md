I love Anki

[Click here to download the Italian Audio Anki Deck ](https://github.com/svenelyes/ankisvenelyes/releases/latest/download/Italiano_Conjugation_With_Audio.apkg)
All kind of code helping me make it eaiser

TTS allows to generate Audio Files for Ultimate Geography Deck
TODO: use a different Text to Speech (maybe open AI) to have nicer audio
This would be important for Cross Language Anki Decks


## LIST of Public Anki Decks:

[GeoDeck](https://ankiweb.net/shared/info/2109889812)


The Italian Deck can be found [here](https://ankiweb.net/shared/info/1891639832) 

[Italain Vocab Page here ](https://www.asiteaboutnothing.net/w_ultimate_italian_conjugation.php#verb-list)

To communicate with Anki, Use [Anki Connect](https://ankiweb.net/shared/info/2055492159)
